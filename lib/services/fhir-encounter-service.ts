...
/**
 * Fetch a patient's encounters (+ their clinical impressions) from Medplum
 */
...
/**
 * Create a new encounter + clinical impression in Medplum.
...
export async function createFhirEncounter(
  ...
  try {
    ...
    if (!encounterRes.ok) {
      return {
        success: false,
        error: encounterData.error || "Failed to create encounter in Medplum",
      };
    }
...
/**
 * Update an existing encounter's note in Medplum (ClinicalImpression upsert).
...
export async function updateFhirEncounter(
  ...
  if (!enc.encounterFhirId) {
    return {
      success: false,
      error: "Encounter has no FHIR ID - cannot update in Medplum",
    };
  }
