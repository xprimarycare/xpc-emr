...
/**
 * Create a new appointment in Medplum
 */
export async function createFhirAppointment(
  ...
  try {
    ...
    if (!res.ok) {
      return {
        success: false,
        error: data.error || "Failed to create appointment in Medplum",
      };
    }
...
/**
 * Update an existing appointment in Medplum
 */
export async function upsertFhirAppointment(
  ...
  if (!appointment.fhirId) {
    return {
      success: false,
      error: "Appointment has no FHIR ID — cannot write back to Medplum",
    };
  }
