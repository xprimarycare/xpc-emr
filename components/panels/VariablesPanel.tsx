...
import { Plus, Pin, RefreshCw, Loader2 } from 'lucide-react';
...
export function VariablesPanel() {
  const { variables, variablesLoading, refreshVariables, addVariable, updateVariable, deleteVariable, toggleVariablePin } = useSidebar();
  ...
  // Find the most recent auto-generated variable's lastRefreshedAt
  const lastRefreshed = Object.values(variables).reduce<number | null>((latest, v) => {
    if (typeof v === 'string' || !v.isAutoGenerated || !v.lastRefreshedAt) return latest;
    return latest === null ? v.lastRefreshedAt : Math.max(latest, v.lastRefreshedAt);
  }, null);
  ...
  // Check if currently editing an auto-generated variable
  const isEditingAutoGenerated = editingName
    ? (typeof variables[editingName] !== 'string' && variables[editingName]?.isAutoGenerated)
    : false;
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto">
            ...
            <div className="mb-3">
              <label className="block text-xs font-semibold text-gray-600 mb-1">
                ...
                {isEditingAutoGenerated && (
                  <span className="ml-2 text-gray-400 font-normal">(auto-generated from FHIR — read-only)</span>
                )}
              </label>
              <textarea
                value={contentInput}
                ...
                rows={5}
                readOnly={!!isEditingAutoGenerated}
                className={`w-full px-3 py-2 border border-gray-200 rounded-md text-sm resize-none focus:outline-none focus:border-blue-400 ${isEditingAutoGenerated ? 'bg-gray-100 cursor-default' : ''}`}
              />
            </div>
            <div className="flex gap-2">
              {!isEditingAutoGenerated && (
              ...
              {editingName && !isEditingAutoGenerated && (
                <button
              ...
              >
                {isEditingAutoGenerated ? 'Close' : 'Cancel'}
              </button>
            </div>
          </div>
        )}
        ...
        {Object.entries(variables)
          ...
          .map(([name, variable]) => {
          const varData = typeof variable === 'string' ? { name, content: variable, isPinned: false, isAutoGenerated: false } : variable;
          ...
          const isAuto = varData.isAutoGenerated || false;
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {varData.icon && <span className="text-sm">{varData.icon}</span>}
